...
Downstream skills that need the policy doc *content* (e.g., `/extract-ruleset`, `/update-ruleset`, `/extract-sample-rules` in its Pass 4b, and `/extract-test-cases`) read the caveman-compressed counterpart at `<domain>/policy_facets/compressed/<rel>.md`. `/extract-sample-rules` and `/extract-test-cases` additionally permit a fallback read of the source doc at `<domain>/input/policy_docs/<rel>.md` only when the compressed text for the targeted section is ambiguous, unclear, or questionable for the rule or example at hand. In both, the canonical citation written back to artifacts (`source_doc:`, `source.file`) is always the source-doc path regardless of which mirror was read.

Downstream skills that need the policy doc *structured section data* (e.g., `/suggest-target-ruleset`, `/create-skeleton`, `/create-ruleset-groups`, `/create-ruleset-modules`, `/extract-sample-rules`, `/refine-guidance`) glob `<domain>/policy_facets/computations/**/*.md.yaml`. Each per-file file is a YAML map with one top-level key: `sections:` — a list of `{heading, summary, tags, stage?, stage_source?, computations?}` section blocks. Each `computations[]` entry carries `description:`, optional `preconditions:`, and an optional `expr_hint:` of the form `output_name = <expression>` (the LHS is a snake_case identifier naming the computed output; the RHS is the bare expression — input names are derived by tokenizing the RHS). Descriptive-only computations omit `expr_hint:`; consumers fall back to scanning `description:` prose for variable names. Consumers read `data["sections"]` only. The source path is encoded in the filename — a per-file file at `policy_facets/computations/<rel>.md.yaml` describes the source at `input/policy_docs/<rel>.md`. Readers reconstitute the `path:` field by stripping the trailing `.yaml` suffix from the per-file file's relative path under `policy_facets/computations/` and prefixing with `input/policy_docs/`. There is no `path:` field inside the per-file files.

**Cross-file naming consistency.** Cross-file naming consistency is best-effort post-confirmation. `specs/naming-manifest.yaml` is the analyst-authoritative source of canonical names, populated by `/declare-target-ruleset` (seeded entries with nullable provenance) and `/extract-ruleset` Step 7 (which fills provenance and optional fields once the analyst confirms a name against an observed phrase). Workers consult the manifest on every run; `core/naming_guide.md` supplies static style rules for deriving fresh names when no manifest entry covers a concept. When `/extract-ruleset` Step 7 writes a rename to `specs/naming-manifest.yaml`, it appends the prior specs key to the entry's `synonyms:` list (carrying the existing `synonyms:` forward so the full rename chain accumulates across rounds). Each entry in `specs/naming-manifest.yaml` carries optional `policy_phrase:`, `source_doc:`, `section:` (all nullable on seeded entries — `/extract-ruleset` Step 7 fills them via the preserve-non-null rule once the analyst confirms a seeded name), plus optional `description:`, `type:`, `values:`, and `synonyms:`. **Descriptive guidance lives in dedicated `guidance/` files** — `output-variables.yaml`, `input-variables.yaml`, `include-with-output.yaml`, `constants-and-tables.yaml`. Structural data lives in `naming-manifest.yaml`, descriptive content split across the four focused files. `xlator validate-guidance <domain>` enforces name-ref alignment and `type:`/`values:` agreement between manifest and guidance files.
...
Typical steps:
  ...
  4. Write an AI prompt to extract a ruleset in the `guidance/` folder — two options:
      ...
      * **Step-by-step (for UI-driven or incremental workflows):**
        ...
        - `/declare-target-ruleset <domain>` — write initial `naming-manifest.yaml` and `guidance/` files from a specified target ruleset file (one of the files created by `/suggest-target-ruleset`)
