...
Downstream skills that need the policy doc *structured section data* (e.g., `/suggest-target-ruleset`, `/create-skeleton`, `/create-ruleset-groups`, `/create-ruleset-modules`, `/extract-sample-rules`, `/refine-guidance`) glob `<domain>/policy_facets/computations/**/*.md.yaml`. Each per-file file is a YAML map with two top-level keys: `naming_manifest:` (a `variables:` map of every variable name surfaced in the file with its verbatim `policy_phrase:`, required `source_doc:`, and optional `role_hint:`, `section:`, `description:`, `type:`, and `values:`) and `sections:` (a list of `{heading, summary, tags, stage?, stage_source?, computations?}` section blocks — same shape as the prior list-of-sections format). Consumers that only care about section data read `data["sections"]`. The source path is encoded in the filename — a per-file file at `policy_facets/computations/<rel>.md.yaml` describes the source at `input/policy_docs/<rel>.md`. Readers reconstitute the `path:` field by stripping the trailing `.yaml` suffix from the per-file file's relative path under `policy_facets/computations/` and prefixing with `input/policy_docs/`. There is no `path:` field inside the per-file files. **Worker invariant on `source_doc:`:** the field's value MUST equal `input/policy_docs/<rel>.md` for the per-file file at `policy_facets/computations/<rel>.md.yaml` (i.e., source_doc is data, not metadata derived from path). The merge tool sanity-checks this; mismatches surface as warnings in the `xlator naming-defaults --build` JSON summary's `errors:` array.

**Cross-file naming consistency.** `/index-inputs` runs `xlator naming-defaults --build` after the per-file extract finalize, merging every per-file `naming_manifest:` block into `<domain>/policy_facets/naming-defaults.yaml` (a flat `variables:` map of canonical names + optional `description:`, `type:`, `values:`, `role_hint:`, `source_doc:`, `section:`, and `synonyms:` — a list of `{name, source_doc, section}` rows recording each observed name and its source, including self-synonym rows when the canonical name itself was observed in multiple files). `type:` is one of `money | bool | int | float | string | enum | list | date`; `values:` is required when `type: enum` and omitted otherwise. AI workers emit `description:` / `type:` / `values:` only on clear source signals — see `xl-plugin/core/naming_guide.md` for the trigger tables and merge-time resolution rules. The merge tool also rewrites in place any per-file `sections[*].computations[*].variables` lists whose names changed during canonicalization, so all downstream consumers see one agreed-upon name per concept. Workers read a three-level authority chain on every run — `specs/naming-manifest.yaml` (highest, **analyst-authoritative**: confirmed against a doc OR seeded pre-extraction by `/declare-target-ruleset`; may also override `description:`/`type:`/`values:`) → `policy_facets/naming-defaults.yaml` (mid, auto-picked) → `core/naming_guide.md` (lowest, static style rules). When `/extract-ruleset` Step 7 writes a rename to `specs/naming-manifest.yaml`, it sets `original_name:` to the prior canonical (preserving the earliest anchor across rename chains); the next `/index-inputs` run flows that rename through the worker authority chain — analysts never copy renames back manually. Each entry in `specs/naming-manifest.yaml` carries optional `policy_phrase:`, `source_doc:`, `section:` (all nullable on seeded entries — `/extract-ruleset` Step 7 fills them via the preserve-non-null rule once the analyst confirms a seeded name against an observed phrase), plus optional `original_name:`, `description:`, `type:`, `values:`, and `synonyms:` (the row list `[{name, source_doc, section}, ...]`) — every optional field except `role_hint:` propagates from the matching `policy_facets/naming-defaults.yaml` entry on initial write or rename-replacement. `role_hint:` is intentionally excluded; entity grouping (`inputs.<Entity>` vs `computed:` vs `outputs:`) encodes role. Name-match re-runs preserve specs entries' non-null fields, so analyst-supplied seed values survive while null/absent fields gap-fill from defaults (v7.0.0 amendment of v6.1.0's preserve-verbatim rule). The phraseless-seeded path: `xlator naming-defaults --build` runs a two-pass merge — pass 1 is the existing phrase-grouped flow; pass 2 walks specs entries with no `policy_phrase` and emits any name not surfaced by pass 1 as a standalone canonical in `naming-defaults.yaml` (no top-level `source_doc:` / `section:`, no `synonyms:`); name-equality reconciliation collapses pass-2 entries into pass-1 canonicals when names match (seeded type/values/description win on conflict). The merge tool emits a convergence warning when a standalone seeded name is similar to an observed canonical (Levenshtein-based) so the analyst can confirm or rename in `/extract-ruleset` Step 3b. **Descriptive guidance lives in dedicated `guidance/` files** — `output-variables.yaml`, `input-variables.yaml`, `include-with-output.yaml`, `constants-and-tables.yaml`. The legacy `guidance/variables.yaml` is gone in v7.0.0; structural data lives in `naming-manifest.yaml`, descriptive content split across the four focused files. `xlator validate-guidance <domain>` enforces name-ref alignment between manifest and guidance files.

**Section-block field semantics.** `stage:` is a snake_case identifier naming the stage of analysis the section belongs to (e.g., `initial_screening`, `deductions`); `/extract-computations` populates it only when the source doc surfaces an explicit phase or stage signal in heading text, body text, or an attributable ancestor heading. `stage_source:` is the verbatim source-text quote that justified the `stage:` identifier — paired with `stage:` (both fields ship together or both are absent). The verbatim quote enables on-demand verification: `grep -F "<stage_source>" <input/policy_docs/<rel>.md>` proves the AI honored the explicit-signal rule. **The per-section `stage:` field is unrelated to CIVIL's `RulesetGroup.description` "evaluation phase" wording in `xl-plugin/core/ruleset.schema.json` / `xl-plugin/tools/civil_schema.py` — CIVIL's "phase" controlled-vocab is a different concept.** The per-section `stage:` is a pre-derivation signal feeding `/create-ruleset-groups` → `guidance/ruleset-groups.yaml` → `/extract-ruleset`; CIVIL's `RulesetGroup` is the compiled output. `stage:` is single-owner: only `/extract-computations` writes it; consumer skills read but never modify or invent it.
