...
```
/xl:extract-test-cases [<domain>]                  # auto-detect program or prompt if ambiguous
/xl:extract-test-cases [<domain> <program>]        # target a specific <program>.civil.yaml
```
...
3. **`input/` has documents?** — If `$DOMAINS_DIR/<domain>/input/` is empty or absent → Print: "No input documents found in `$DOMAINS_DIR/<domain>/input/`. Add policy documents and re-run." Stop.
...
```yaml
# Auto-generated by /xl:extract-test-cases. Do not edit directly —
# re-run /xl:extract-test-cases and choose "re-extract" to refresh.
extracted_tests:
  - case_id: "ext_001"
