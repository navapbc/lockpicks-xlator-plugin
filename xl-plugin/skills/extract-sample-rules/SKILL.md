...
- **When a section has an explicit `stage:` value, bind the rule's CIVIL `group:` annotation to that stage** — apply the same suffix-stripping normalization as `/create-ruleset-groups` so the `group:` value appears in `ruleset_groups[*].name` and `validate_civil.py` accepts the rule. Do not derive `group:` from heading text when `stage:` is present; explicit doc signal beats inference
