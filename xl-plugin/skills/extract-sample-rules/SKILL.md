...
- The bare expression (the `expr_hint:` RHS with the `<output> =` prefix stripped) as the `definition` body, substituting canonical names for any input variable tokens and rewriting CIVIL-flavored operators into Catala forms (e.g., `* 0.5` → `* 50%`, `[a, b]` → `[ a; b ]`, money literals as `$1,234`). See `../../core/catala-authoring-quickref.md` Part 2 (money/date literals, list operators) and Part 4 (confabulation traps).
