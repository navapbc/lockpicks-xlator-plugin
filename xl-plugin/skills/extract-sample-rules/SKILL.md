...
- **Do not attach `#[error.message = ...]` to `definition` blocks.** The Catala 1.1.0 compiler only accepts that attribute before `assertion` / `impossible`; on a `definition` it warns and silently drops the tag. See `../../core/catala-authoring-quickref.md` §3.4. Stage tagging at the `definition` level is therefore not emitted; if an assertion is generated downstream for the same stage, that is the correct attachment site.
...
- **Do not attach `#[error.message]` to `definition` blocks** — the Catala 1.1.0 compiler only accepts that attribute before `assertion` / `impossible`. Stage-derived tagging at the definition level is dropped silently and produces a compiler warning. See `../../core/catala-authoring-quickref.md` §3.4.
