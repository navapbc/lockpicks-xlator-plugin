...
- **`description`, `type`, `optional`, `values`, `enum_variants`**: optional analyst- or AI-supplied values. AI-infer them from policy-doc context plus the inferred Catala scope-declaration shape:
  - `type:` — Catala-native (`integer`, `decimal`, `money`, `boolean`, `date`, `duration`, `string`, `enum`, `list`, `structure`) from currency markers ("$", "dollars" → `money`), yes/no phrasing → `boolean`, enumerated lists → `enum`, calendar dates → `date`, repeated values → `list`, compound records → `structure`, and so on. The merge tool rejects any other value — including legacy CIVIL names (`bool`, `int`, `float`, `str`, `set`, `object`).
