...
- **`policy_facets/naming-defaults.yaml`** — domain-wide canonical-name map merged from every per-file `naming_manifest:` block, with auto-picked canonicals, observed synonyms, and `role_hint:` resolution. Rewritten on every run by `xlator naming-defaults --build` (the new finalize step after `--finalize` of compress + extract).
