...
- **`policy_facets/naming-defaults.yaml`** — domain-wide canonical-name map merged from every per-file `naming_manifest:` block, with auto-picked canonicals, per-name source rows (one row per (name, file, section) observation, including self-synonym rows when the canonical name was observed in multiple files), and `role_hint:` resolution. Rewritten on every run by `xlator naming-defaults --build` (the new finalize step after `--finalize` of compress + extract).
...
### Step 7: Build naming defaults

After both `--finalize` calls complete and the per-file `policy_facets/computations/<rel>.md.yaml` files reflect this run, merge their `naming_manifest:` blocks into `policy_facets/naming-defaults.yaml`:

```bash
xlator naming-defaults <domain> --build
```

The tool reads every `policy_facets/computations/**/*.md.yaml` file (including SKIP'd files from prior runs — the merge is the union of all per-file files, not just the ones re-extracted in this run), the optional `specs/naming-manifest.yaml` (highest-authority canonical names), and rewrites `policy_facets/naming-defaults.yaml` with auto-picked canonicals + synonyms. It also rewrites in place any per-file `sections[*].computations[*].variables` lists whose names changed during canonicalization.

The tool's stdout is a JSON summary: `{"merged": <N>, "synonyms_collapsed": <M>, "files_rewritten": <K>, "canonicals_changed": [...], "errors": [...]}`.

Relay the summary inside its own `:::important` block:

:::important
✓ Naming defaults: <merged> canonical, <synonyms_collapsed> synonyms collapsed, <files_rewritten> per-file file(s) rewritten, <C> canonical(s) changed since prior run.
:::

`<C>` is `len(canonicals_changed)`. When `<C>` > 0, list each rename on its own line inside the same block:

```
  <from> → <to>  (phrase: "<phrase>")
```

If the tool exits non-zero, surface its stderr inside `:::error` and exit non-zero. Do NOT roll back prior finalize artifacts — the per-file files remain valid; `naming-defaults.yaml` may be missing or partial and will be rebuilt on the next run.

### Step 8: Completion

After Step 7 completes, print:
...
✓ policy_facets/input-index.yaml written (files block).
  ...
  policy_facets/naming-defaults.yaml written.
...
### Step 9: Build naming defaults

Same shape as CREATE Step 7: run `xlator naming-defaults <domain> --build`, relay its JSON summary inside `:::important`, list any `canonicals_changed` renames, surface the error block on non-zero exit.

The merge is the union of every `policy_facets/computations/**/*.md.yaml` (including SKIP'd files), so a re-run that REINDEX'd only a subset still produces a `naming-defaults.yaml` covering the whole domain.

### Step 10: Completion

After Step 9 completes, print:
...
✓ policy_facets/input-index.yaml updated (files block).
  ...
  policy_facets/naming-defaults.yaml updated.
  <R> file(s) rejected (low quality) and moved to input/rejected/ — re-run after reformatting.
:::
...
- **Don't manually edit `policy_facets/naming-defaults.yaml`** — it is regenerated on every run by `xlator naming-defaults --build`. To change a canonical name, rename it in `specs/naming-manifest.yaml` (with `original_name:`); the next `/index-inputs` run flows the rename through the worker authority chain and the merge tool.
