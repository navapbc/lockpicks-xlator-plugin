...
- **`policy_facets/computations/<rel>.md.yaml`** (one file per source doc) — per-file YAML lists of `{heading, summary, tags, computations}` blocks. Generated by `/extract-computations` per file as part of the unified per-file batch.
...
After both finalizes complete, print:
