...
3. **Transpiled output exists?**
   ...
   - Not found → Print:
     :::error
     No .rego file found. Run /transpile-and-test <domain> <module> first.
     :::
     Stop.
...
```bash
# Read: core/demo/rego.html.template
```
...
:::next_step
Next steps:
  1. Install deps:   uv pip install -r $DOMAINS_DIR/<domain>/output/demo-rego-<module>/requirements.txt
